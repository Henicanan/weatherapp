...
const closeIfClickOutside = (event: MouseEvent | TouchEvent) => {
  ...
  if (!target.closest('.search-section') && !target.closest('.result-wrapper') && !target.closest('.example__city')){
    isActive.value = false
  }
}

function detectLanguage(input: string): 'ru' | 'en' | 'mixed language' | 'unknown language' {
...
function getCitiesByDetectedLanguage(
  ...
  return lang === 'ru' || lang === 'en' ? filterCitiesByLanguage(cities, lang) : [];
}
...
watch(inputData, (newVal) => {
  ...
  timeoutId = setTimeout(() => {
    if (newVal.length >= 3) {
      const citiesByLang = getCitiesByDetectedLanguage(citiesArray.value, newVal);
...
<template>
    <div class="search-section" @pointerdown="isActive = true">
        <input ref="inputRef" v-model="inputData" class="search" type="text" placeholder="Укажите город"
        @keydown.enter.prevent>
    </div>
    <div  ref="resultWrapperRef" class="result-wrapper">
        <ul v-if="isActive" class="result-list" v-auto-animate>
            <li class="result-item" v-for="(city, index) in resultArray" :key="index">
                <NuxtLink class="result-item__link" 
                :to="getCityLink(city)">
